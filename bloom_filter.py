...
class BloomFilter:
    ...
    def _hashes_universal(self, item):
        """Universal Hashing: Use random primes and seeds to generate the hash values."""
        result = []
        prime = 31
        for i in range(self.hash_count):
            seed = random.randint(1, self.size)
            hash_result = (murmurhash.hash(item + str(seed)) * prime + i) % self.size
            result.append(hash_result)
        return result
